const createActions = ({ actions, resource, client, only, parseList, parseSingle, parseError }) => {
  const crudActions = {};
  ...
  if (only.includes('FETCH_LIST')) {
    Object.assign(crudActions, {
      ...
      fetchList({ commit }, { config } = {}) {
        ...
        return client.findAll(resource, config)
          .then((res) => {
  ...
  if (only.includes('FETCH_SINGLE')) {
    Object.assign(crudActions, {
      ...
      fetchSingle({ commit }, { id, config } = {}) {
        ...
        return client.find(resource, id, config)
          .then((res) => {
  ...
  if (only.includes('CREATE')) {
    Object.assign(crudActions, {
      ...
      create({ commit }, { data, config } = {}) {
        ...
        return client.create(resource, data, config)
          .then((res) => {
            const parsedResponse = parseSingle(res);
  ...
  if (only.includes('UPDATE')) {
    Object.assign(crudActions, {
      ...
      update({ commit }, { id, data, config } = {}) {
        ...
        return client.update(resource, data, config)
          .then((res) => {
            const parsedResponse = parseSingle(res);
  ...
  if (only.includes('DESTROY')) {
    Object.assign(crudActions, {
      ...
      destroy({ commit }, { id, config } = {}) {
        ...
        return client.destroy(resource, id, config)
          .then((res) => {
            const parsedResponse = parseSingle(res);
